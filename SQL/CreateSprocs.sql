...
BEGIN
	SELECT A.ArticleTitle, A.DateAdded, A.ArticleContent, A.ArticleId, ANU.Id, A.Approved, ANU.UserName
	FROM Article A
	--left join ItemArticle IA on A.ArticleId = IA.ArticleId
	--left join Item I on IA.ItemId = I.ItemId
	left join AspNetUsers ANU on A.UserId = ANU.Id 
...
BEGIN
	SELECT top 8 ItemId, ItemName, ItemPictureURL, DisplayValue, RealValue, [Description], Favorite, Featured
	FROM Item
	Where Featured = 1
	ORDER BY DisplayValue DESC
END
...
BEGIN
	SELECT ItemId, ItemName, ItemPictureURL, DisplayValue, RealValue, [Description], Favorite, Featured
	FROM Item
where ItemId = @ItemId
ORDER BY DisplayValue DESC
END
...
BEGIN
	SELECT ANU.UserName, ANU.Id AS UserId, ANU.Email, ANR.Name
